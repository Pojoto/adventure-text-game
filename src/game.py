...
class Game:
    ...
    def __init__(self):
        ...
        self.verbs = {'climb', 'close', 'dig', 'drop', 'examine', 'go',
            ...
            'swim', 'take', 'touch', 'type', 'unlock', 'eat', 'health'} #NEWSTUFF - 'eat', 'health' verbs added
        
        #NEWSTUFF - removed 'self.inventory', added name input and player object creation
    ...
    def process_command(self, command):
        ...
        if verb == 'go':
            ...
            #NEWSTUFF - process_inventory() is now called from a player object
            self.player.process_inventory(self.items)
        elif verb == 'look':
            self.print_room(self.current_room)
        elif verb == 'examine':
            ...
            self.process_dig()
        elif verb == 'eat': #NEWSTUFF - added eat verb function support
            self.process_eat(words)
        elif verb == 'health':
            self.process_health() #NEWSTUFF - added health verb function support
    ...
    ##NEWSTUFF - removed process_inventory function (moved to player class)
    ...
    def process_take(self, words):
        if len(words) == 1:
        ...
        elif words[1] in self.player.inventory: #NEWSTUFF - self.inventory -> self.player.inventory
            ...
            if item.name in room.items_in_room:
                ...
                elif not self.player.can_take_item(item, self.items): #NEWSTUFF - utilized can_take_item to check if player can carry an item
                ...
                for item_name_in_room in room.items_in_room:
                    if item_name_in_room in self.containers:
                        ...
                        if item.name in container.contents and container.status == 'open':
                            if not self.player.can_take_item(item, self.items): #NEWSTUFF - utilized can_take_item to check if player can carry an item
                                print("You can't carry it with your current load.")
                            else:
                                self.player.add_item(item.name) #NEWSTUFF - used add_item method to add to players inventory
    ...
    def process_drop(self, words):
        if len(words) == 1:
            ...
            print("I don't know what that is.")
        elif words[1] not in self.player.inventory: #NEWSTUFF - self.inventory -> self.player.inventory
    ...
    def process_examine(self, words):
        if len(words) == 1:
            ...
            item = self.items.get_item(words[1])
            room = self.rooms.get_room(self.current_room)
            if item.name not in room.items_in_room and item.name not in self.player.inventory: #NEWSTUFF - self.inventory -> self.player.inventory
                print("I don't see that here.")
            else:
                print(item)
    ...
    def process_unlock(self, words):
        ...
        if len(words) == 1:
            ...
            if item.status == 'open':
                ...
                elif 'key' not in self.player.inventory: #NEWSTUFF - self.inventory -> self.player.inventory
    ...
    def process_put(self, words):
        ...
        if len(words) < 4:
        ...
        elif words[3] not in room.items_in_room and words[3] not in self.player.inventory: #NEWSTUFF - self.inventory -> self.player.inventory
            ...
            if container.name not in self.containers:
                ...
                self.player.remove_item(item.name) #NEWSTUFF - used player's remove_item method to remove an item
    ...
    def process_read(self, words):
        ...
        if len(words) == 1:
            ...
            print("I don't know what that is.")
        elif words[1] not in self.player.inventory and words[1] not in room.items_in_room: #NEWSTUFF - self.inventory -> self.player.inventory
            print("I don't see that here.")
        elif words[1] == 'book':
            if 'book' not in self.player.inventory: #NEWSTUFF - self.inventory -> self.player.inventory
    ...
    def process_dig(self):
        ...
        elif 'spoon' not in self.player.inventory: #NEWSTUFF - self.inventory -> self.player.inventory
    ...
    def print_welcome(self):
        print(f'Welcome to Snarfblat, {self.player.name}!\n') #NEWSTUFF - added player name to welcome message
        print('Man, this needs a better name.\n')
    ...
    def print_ghost_visit(self, painting_status):
        ...
        print(f'"Help me, {self.player.name}!" she moans. "Retrieve my treasures! Set things right!"') #NEWSTUFF - added player name to ghost dialog
    ...
    def print_winner(self):
        ...
        print(f'"You did it {self.player.name}! Thank you for retrieving my precious things!" she says.') #NEWSTUFF - added player name to ghost dialog
